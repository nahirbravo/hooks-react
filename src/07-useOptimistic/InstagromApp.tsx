...
export const InstagromApp = () => {
 const [isPending, startTransition] =  useTransition()
  ...
  const [optimisticCommments, addOptimistcComment] = useOptimistic( comments, 
     ( currentComents, newCommentText: string) => {
      lastId++;
      return[...currentComents, {
        id: lastId,
        text: newCommentText,
        optimistic: true 
  ...
  const handleAddComment = async (formData: FormData) => {
    ...
    addOptimistcComment(messageText);
    ...
    //siuladnso que la peticion falla
    //CODIGO PARA REVERTIR EL PROCDESO SI LA RENDERIZACION OOPTIMISTA FALLA
    setComments((prev) => prev);
    toast('error al agregar el comentraio', {
      description: 'intente nuevamenete',
      duration: 10_000,
      position: 'top-right',
      action:{
        label: 'cerrar',
  ...
  return (
    <div className="bg-slate-700 h-screen flex flex-col items-center justify-center">
      ...
      <ul className="flex flex-col items-start justify-center bg-gray-300 w-[500px] p-4">
        {optimisticCommments.map((comment) => (
